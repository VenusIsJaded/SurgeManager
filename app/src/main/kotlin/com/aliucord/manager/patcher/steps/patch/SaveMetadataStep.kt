...
class SaveMetadataStep(private val options: PatchOptions) : Step(), KoinComponent {
    ...
    override suspend fun execute(container: StepRunner) {
        ...
        val surgeXposed = container.getStepOrNull<DownloadSurgeXposedStep>()
        ...
        val metadata = InstallMetadata(
            ...
            surgeXposedVersion = surgeXposed?.targetVersion,
        )

        container.log("Writing serialized install metadata to APK")
        ZipWriter(apk, /* append = */ true).use {
            it.writeEntry("surge.json", json.encodeToString<InstallMetadata>(metadata))
        }
    }
}
