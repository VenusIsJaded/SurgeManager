 ...
 * Also carries the raw [statusMessage] from [PackageInstaller.EXTRA_STATUS_MESSAGE]
 * so it can be surfaced in logs and the error stacktrace.
 */
@Parcelize
data class PMInstallerError(
    val status: Int,
    val statusMessage: String? = null,
) : InstallerResult.Error(), Parcelable {

    override fun getDebugReason() = buildString {
        append(when (status) {
