...
import com.aliucord.manager.patcher.SurgePatchRunner
...
class PatchingScreenModel(
    ...
    // Version info extracted before running
    val discordVersionString: String? = run {
        val localPath = options.localApkPath
        if (!localPath.isNullOrBlank()) {
            options.customVersionCode
                ...
                ?: run {
                    ...
                    val info = file
                        .takeIf { it.exists() }
                        ?.let { extractApkFileInfo(application, it) }

                    info?.let { "v${it.versionName} (${it.versionCode})" }
                }
        } else if (options.customVersionCode.isNotBlank()) {
            "v${options.customVersionCode}"
        } else {
            null
        }
    }
    ...
    private suspend fun startPatchRunner() {
        val runner = SurgePatchRunner(options)
            .also { stepRunner = it }
